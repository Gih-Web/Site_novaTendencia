...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['listar_por_categoria'])) {
  ...
  if ($catId <= 0) {
    echo json_encode(['ok' => false, 'erro' => 'idCategoria inválido']);
    exit;
  }

  try {
    // Consulta compatível com seu banco atual
    $sql = "SELECT
              p.idProdutos,
              p.nome,
              p.descricao,
              ...
              m.nome AS marca,
              c.nome AS categoria,
              -- pega apenas a primeira imagem associada
              (SELECT ip.foto
            ...
            FROM PRODUTOS p
            LEFT JOIN MARCAS m ON m.IdMarcas = p.marcas_id
            INNER JOIN PRODUTO_CATEGORIA pc ON pc.produtos_id = p.idProdutos
            INNER JOIN CATEGORIA c ON c.idCategoria = pc.categoria_produtos
            WHERE pc.categoria_produtos = :catId
            GROUP BY p.idProdutos
            ORDER BY p.idProdutos DESC";
    ...
    echo json_encode([
      'ok' => false,
      'erro' => 'Falha ao listar produtos por categoria',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  try {
    ...
    if ($precoPromo === null)
      $stmt->bindValue(':precoPromo', null, PDO::PARAM_NULL);
    else
      $stmt->bindValue(':precoPromo', $precoPromo);
    $stmt->bindValue(':marcaId', $marcaId, PDO::PARAM_INT);
    $stmt->execute();

    $produtoId = (int)$pdo->lastInsertId();

    // 2. Inserir imagens (se houver)
    $imagens = [
